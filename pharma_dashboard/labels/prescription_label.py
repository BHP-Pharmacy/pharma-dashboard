...
from pharma_subject.constants import TABLET, SYRUP, IM, IV, SUPPOSITORY, \
    SOLUTION, CAPSULE
...
class PrescriptionLabel(Label):
    ...
    def __init__(self, dispense=None, item=None, user=None,
                 label_template_name=None):
    ...
    def label_context(self):
        ...
        try:
            patient = patient_cls.objects.get(
                subject_identifier=self.dispense.subject_identifier)
            ...
            label_context = {
                ...
                'prepared_datetime': self.dispense.prepared_datetime.strftime(
                    "%d-%m-%y %H:%M"),
                'prepared_by': profile_cls.objects.get(
                    user__username=self.user.username).initials,
            ...
            if self.dispense.dispense_type in [TABLET, CAPSULE, SUPPOSITORY]:
                ...
                elif self.dispense.medication.protocol.name.upper() == 'HPTN 084':
                    label_context.update({
